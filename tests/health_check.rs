...
async fn health_check_works() {
  ...
  // Either add it manually under [dev-dependencies] in Cargo.toml // or run `cargo add reqwest --dev`
...
// `cargo add tokio --dev`
let server = rust_subscribe::run().expect("Failed to bind address"); // Launch the server as a background task
